...
const selected_type = ref<Type | null>(null)
...
const deleteRecord = async () => {
  ...
  try {
    const response = await $fetch(`/api/types/deleteType/${selected_type.value.id}`, {
      method: "DELETE"
    })
...
<template>
  <v-container>
    ...
    <v-row class="mt-10">
      <v-col>
        <v-table v-if="types" height="300px">
          <thead>
            <tr>
              <th>#</th>
              <th>Nombre</th>
            ...
            <tr v-for="item in types">
              ...
              <td>
                <v-btn variant="flat"
                       ...
                       title="Editar"
                       size="small"
                       @click="router.push(`/types/${item.id}`)"
                >
                ...
                <v-btn variant="flat"
                       ...
                       size="small"
                       class="ml-2"
                       @click="selected_type=item; show_confirm=true"
                >
    ...
    <v-dialog v-model="show_confirm" max-width="400">
      <v-card style="padding: 20px">
        <v-card-title class="text-h6">¿Eliminar {{selected_type.name}}?</v-card-title>
