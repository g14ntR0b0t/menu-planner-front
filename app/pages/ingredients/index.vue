...
const submit = async () => {
  ...
  if (valid) {
    ...
    formData.append('measure_unit', String(measure_unit_selected.value.id))
    // Append image if exists
    if(image.value) formData.append('image', image.value)
    try{
      const config = useRuntimeConfig()
      const response = await $fetch(`${config.public.apiBase}/api/ingredients/`, {
        method: 'POST',
        body: formData
      })
      success.value =  true
      ...
      let msg = ''
      console.error(err?.data?.data?.description)
      err?.data?.data?.description.forEach(
          (annotation) => {
            msg += annotation
          }
      )
